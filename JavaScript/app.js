...
const cookiesform = document.getElementById('Sales');
cookiesform.addEventListener('submit', data);
function data(event) {
  event.preventDefault();
  const name = event.target.name.value;
  console.log(name);
  const minimum = event.target.min.value;
  console.log(minimum);
  const maximum = event.target.max.value;
  console.log(maximum);
  const average = event.target.avg.value;
  console.log(average);
  const newObject = new Location(name, minimum, maximum, average);
  console.log(newObject);
  newObject.calCustomersEachHour();
  newObject.calcCookiesEachHour();
